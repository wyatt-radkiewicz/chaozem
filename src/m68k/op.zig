    ...
    return struct {
        pub fn op(ctx: *Ctx, comptime _: Size, src: u3, dst: u3) void {
            ...
            switch (@as(u2, @intFromEnum(src_reg)) << 1 | @intFromEnum(dst_reg)) {
                ...
                0b10 => std.mem.swap(u32, ctx.cpu.r(.a, src), &ctx.cpu.d[dst]),
                0b01 => std.mem.swap(u32, &ctx.cpu.d[src], ctx.cpu.r(.a, dst)),
                0b11 => std.mem.swap(u32, ctx.cpu.r(.a, src), ctx.cpu.r(.a, dst)),
            }
        }
    };
}
...
pub const Link = struct {
    pub fn op(ctx: *Ctx, comptime _: Size, src: u16, dst: u32) u32 {
        ...
        const stack = ctx.cpu.r(.a, 7).*;
        ctx.cpu.r(.a, 7).* +%= int.extend(u32, src);
        return stack;
    }
};
