...
pub fn PostInc(n: u4) type {
    return struct {
        ...
        pub fn load(this: @This(), ctx: *Ctx, comptime size: Size, _: u16) size.Int() {
            const addr = ctx.cpu.r(.a, this.n).*;
            ctx.cpu.r(.a, this.n).* +%= size.bits() / 8;
...
pub fn RegReg(comptime m: u4, comptime n: u4, comptime delay: RegRegDelay) type {
    return struct {
        ...
        pub fn decode(ctx: *Ctx, comptime size: Size, opcode: u16) @This() {
            const mode = int.extract(Cpu.Reg, opcode, m);
        ...
        pub fn load(this: @This(), ctx: *Ctx, comptime size: Size, _: u16) size.Int() {
            return switch (this.mode) {
                .d => @truncate(ctx.cpu.d[this.reg]),
                .a => ctx.read(size.Int(), ctx.cpu.r(.a, this.reg).*),
            };
        }

        pub fn store(this: @This(), ctx: *Ctx, comptime size: Size, _: u16, data: size.Int()) void {
            switch (this.mode) {
                .d => ctx.cpu.d[this.reg] = int.overwrite(ctx.cpu.d[this.reg], data),
                .a => ctx.write(size.Int(), ctx.cpu.r(.a, this.reg).*, data),
            }
        }
    ...
    return struct {
        ...
        pub fn load(this: @This(), ctx: *Ctx, comptime size: Size, _: u16) size.Int() {
            return switch (this.mode) {
                ...
                .addr_reg => @truncate(ctx.cpu.r(.a, @truncate(this.addr)).*),
        ...
        pub fn store(this: @This(), ctx: *Ctx, comptime size: Size, _: u16, data: size.Int()) void {
            switch (this.mode) {
                ...
                .addr_reg => ctx.cpu.r(.a, @truncate(this.addr)).* =
                    int.overwrite(ctx.cpu.r(.a, @truncate(this.addr)).*, data),
                .immediate => {},
        ...
        pub fn decode(ctx: *Ctx, comptime size: Size, opcode: u16) @This() {
            ...
            const base = switch (mode) {
                .post_inc => post_inc: {
                    ...
                    ctx.cpu.r(.a, reg).* +%= @intCast(mask.count() * size.bits() / 8);
                    break :post_inc addr;
                },
                .pre_dec => pre_dec: {
                    ctx.cpu.r(.a, reg).* -%= @intCast(mask.count() * size.bits() / 8);
                ...
                else => mode.calc(ctx, size, reg),
            };
            switch (op) {
                .load => mask.load(size, ctx, base),
                .store => mask.store(size, ctx, base),
                .addr => {},
            }
            return .{};
        }
...
pub fn Peripheral(comptime addr_n: u4) type {
    return struct {
        ...
        pub fn load(this: @This(), ctx: *Ctx, comptime size: Size, _: u16) size.Int() {
            var addr = ctx.cpu.r(.a, this.reg).* +% this.disp;
            var val: size.Int() = 0;
